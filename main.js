...
function more(i){
    ...
    var origin_price = cards[i].discountPercentage ? (cards[i].price*3.21/(1-cards[i].discountPercentage/100)).toFixed(3) +  "<sup>DT</sup>" : "";
    var qte_int = cards[i].stock >= 1 ? 1 : 0; 
    var link = "<a href=\"#cards\" class=\"btn btn-primary\" onclick=\"index()\">back</a>"
    var text = "<div class=\"col\"><div> " + link + " </div><img src= \"" + cards[i].images[0] + "\" alt = " + cards[i].title + " class=\"col-10\" ></div><div class=\"col bg-light\"><h1 class=\"card-title\">"+ cards[i].title+" </h1><h5 class=\"card-text text-secondary \">Réf: MQAM3HX/A"+ cards[i].id + "</h5><p>"+cards[i].description+"</p><div class=\"d-flex  mb-3\"><h4 class=\"card-text text-black \"><del>"+ origin_price + "</del></h4>&nbsp;<h1 class=\"card-text text-danger \">"+ (cards[i].price*3.21).toFixed(3) + "<sup>DT</sup></h1></div><div class=\"text-center d-flex justify-content-between\"><div class=\"d-flex justify-content-between w-50\"><label for = \"Qte#"+ i +"\"> <h4 class=\" text-success\" >Quantité :</h4> </label><div class=\"w-50\" ><input type=\"number\" id = \"Qte#"+ i +"\" value=\""+qte_int+"\" min=\"0\" max="+ cards[i].stock+" class=\"form-control\"></div></div><a href=\"#\" class=\"btn btn-outline-primary\" onclick=\"addPanier("+ i +",x = parseInt(document.getElementById('Qte#"+i+"').value))\" >Ajouter au panier <i class=\"fa-solid fa-bag-shopping text-outline-primary\"></i></a></div></div>"
